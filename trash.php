...
<head>
    ...
    <title>Trash Info | UTB Resto</title>
    ...
    <link rel="stylesheet" href="./charts/css/recyclebin.css">
    <link rel="stylesheet" href="./charts/css/form.css">
...
<h4><strong>Recyclebin</strong></h4>
...
<!-- <table class="table table-bordered">  -->
 ...
 <table class="table-bordered table pull-right" id="mytable" cellspacing="0" style="width: 100%;">
  ...
  <?php
        ...
        $total_pages_sql = "SELECT COUNT(*) FROM recyclebin";
        $result = mysqli_query($con,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT * FROM recyclebin order by register_date DESC LIMIT $offset, $no_of_records_per_page";
        ...
        while($row = mysqli_fetch_array($res_data)){
            //here goes the data
                     ...
                     $user_id = $row['recyclebin_id'];
                     $name = $row['customer_name'];
                     $lunch = $row['payed_lunch'];
                     $breakfast = $row['payed_breakfast'];
                     $dinner = $row['payed_dinner'];
                     $remain_br = $row['remain_breakfast'];
                     $remain_lun = $row['remain_lunch'];
                     $remain_din = $row['remain_dinner'];
                     $money = $row['money_payed'];
                     $user_id = $row['user_id'];
                     $date = $row['register_date'];

            $sel_user = mysqli_query($con, "select * from users where user_id = '$user_id'");
            while($row = mysqli_fetch_array($sel_user)){
                    $fname = $row['user_fname'];
                    $lname = $row['user_lname'];
                    $user_mail = $row['user_email'];
                    $fin_name = $fname . " " . $lname;
                    $i++;
  ...
  <tr style="text-align: right;">
                ...
                <td><?php echo $remain_br; ?></td>
                <td><?php echo $remain_lun; ?></td>
                <td><?php echo $remain_din; ?></td>
                <td><?php echo $money; ?></td>
                <td style="text-transform: capitalize;"><?php echo $fin_name; ?></td>
                <td class="lower"><?php echo $user_mail; ?></td>
                <td><?php echo $date; ?></td>
